...
/// The archive format version this build writes and reads: the identity
/// frame's version byte, mirrored by `tarzan_version` in the TOC JSON. The
/// two must agree in every archive.
pub const FORMAT_VERSION: u8 = IDENTITY_VERSION_V2;
...
pub fn identity_frame() -> Vec<u8> {
    encode_skippable_frame(
        &[
            ...
            &[FRAME_TYPE_IDENTITY, FORMAT_VERSION],
        ]
        .concat(),
    )
}
